...
class NLIConnector(BaseConnector):
    ...
    async def health_check(self) -> bool:
        ...
        try:
            await self.predict("The sky is blue.", "The sky is red.")
            return True
        except Exception:
            return False
    ...
    async def predict(self, premise: str, hypothesis: str) -> dict[str, float]:
        ...
        scores = await asyncio.to_thread(
            self._model.predict, [(premise, hypothesis)]
        )
        return dict(zip(self.LABELS, scores[0].tolist()))
    
    async def is_contradicting(self, a: str, b: str) -> bool:
        """
        ...
        """
        result = await self.predict(a, b)
        return result["contradiction"] >= config.nli.contradiction_threshold
    
    async def is_entailing(self, a: str, b: str) -> bool:
        """
    ...
    async def predict_batch(self, pairs: list[tuple[str, str]]) -> list[dict[str, float]]:
        """
    ...
    async def find_contradictions(
        ...
        # Batch predict
        results = await self.predict_batch(pairs)
